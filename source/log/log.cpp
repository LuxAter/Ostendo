...
void ostendo::log::Log(unsigned int type, std::string msg, std::string func,
                       ...) {
  std::string str;
...
int ostendo::log::GetLogSize() { return global_logs.size(); }

void ostendo::log::ClearLogs() { global_logs.clear(); }

std::string ostendo::log::GetLog(unsigned int type) {
  std::string entry;
...
std::pair<unsigned int, std::string> ostendo::log::FGetLog(unsigned int type) {
...
std::string ostendo::log::IGetLog(int index) {
  std::string entry;
...
std::pair<unsigned int, std::string> ostendo::log::IFGetLog(int index) {
  std::pair<unsigned int, std::string> entry;
  if (index >= 0 && (size_t)index < global_logs.size()) {
...
std::vector<std::string> ostendo::log::VGetLog(int start, int end) {
...
std::vector<std::pair<unsigned int, std::string>> ostendo::log::VFGetLog(
    int start, int end) {
...
void ostendo::log::SetLogHandle(
    void (*handle)(std::pair<unsigned int, std::string>)) {
  log_handle_full = handle;
}

void ostendo::log::SetLogHandle(void (*handle)(std::string)) {
  log_handle = handle;
}

void ostendo::log::SetLogOption(unsigned int option, int setting) {
...
std::string ostendo::log::GetTypeStr(unsigned int type) {
  std::string str;
  if (type == FATAL) {
    str = "FATAL";
  } else if (type == ERROR) {
...
void ostendo::log::SaveLog(std::string file) {
