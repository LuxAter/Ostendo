...
void ostendo::Menu::PushBackItem(std::string item, size_t col) {
  if (col >= items_.size()) {
    items_.resize(col);
  }
...
void ostendo::Menu::InsertItem(std::string item, size_t position, size_t col) {
  if (col >= items_.size()) {
    items_.resize(col);
  }
...
void ostendo::Menu::InsertColumn(std::vector<std::string> col,
                                 size_t position) {
...
bool ostendo::Menu::MoveSelection(int key) {
  ...
  if (key == KEY_UP &&
    ...
    if (items_[position_[0]][position_[1]].second == std::string()) {
      position_[1] = revert;
             ...
             position_[1] < (int)items_[position_[0]].size() - 1) {
    moved = true;
    int revert = position_[1];
    bool out = false;
    position_[1] += 1;
    while (position_[1] < (int)items_[position_[0]].size() - 1 &&
           items_[position_[0]][position_[1]].second == "") {
      position_[1] += 1;
    }
  ...
  } else if (key == KEY_RIGHT && position_[0] < (int)items_.size() - 1 &&
             position_[1] != -1) {
    int revert = position_[0];
    moved = true;
    position_[0] += 1;
    while (position_[0] < (int)items_.size() - 1 &&
           (items_[position_[0]].size() == 0 ||
            items_[position_[0]][position_[1]].second == std::string())) {
      position_[0] += 1;
    }
...
void ostendo::Menu::Display() {
  ...
  unsigned int begin = 0, end = items_.size();
  win_.GetPosition();
  if (win_.GetPosition().w > (int)items_.size() * width && items_.size() > 0) {
    width = win_.GetPosition().w / items_.size();
  } else if (win_.GetPosition().w < (int)items_.size() * width) {
  ...
  int x = 0;
  for (unsigned int i = begin; i < items_.size() && i < end; i++, x += width) {
...
void ostendo::Menu::DisplayCol(int col, int x, int width) {
  int height = 2;
  unsigned int begin = 0, end = items_[col].size();
  if (win_.GetPosition().h > (int)items_[col].size() * height &&
      items_[col].size() > 0) {
...
int ostendo::Menu::Longest(int col) {
  ...
  if (col == -1) {
    for (size_t j = 0; j < items_.size(); j++) {
      for (size_t i = 0; i < items_[j].size(); i++) {
        longest = (std::max)(longest, win_.PrintSize(items_[j][i].second));
      }
    }
  } else if (col >= 0 && col < (int)items_.size()) {
    for (size_t i = 0; i < items_[col].size(); i++) {
...
int ostendo::Menu::ItemIndex() {
  int item = 0;
  bool found = false;
  for (size_t i = 0; i < items_.size() && found == false; i++) {
    for (size_t j = 0; j < items_[i].size() && found == false; j++) {
      if (std::array<int, 2>{{(int)i, (int)j}} == position_) {
        found = true;
        break;
      }
